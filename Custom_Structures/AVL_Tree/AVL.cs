...
public class AVL<T> where T : IComparable<T>
{
    ...
    private Node<T> Insert(Node<T> node, T item)
    {
        ...
        if (balance > 1)
        {
            ...
            //check if it is with a different sign (opposite to the above) 
            if (childBalance < 0)
            {
        ...
        //this is exact copy of the above
        else if (balance < -1)
        {
    ...
    private Node<T> RotateLeft(Node<T> node)
    {
        ...
        node.Height = 1 + Math.Max(this.Height(node.Left), this.Height(node.Right)); // why?! 
        newRoot.Height = 1 + Math.Max(this.Height(newRoot.Left), this.Height(newRoot.Right));
